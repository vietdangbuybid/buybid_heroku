...
loop do
	market_settings[:categories].each do |category|
		begin
			...
			yahoo_shop_search = market_agent.search_products(page_size, page_index, { market_category_id: category[:yahoo_shop_category_id], query: '' })
			next unless yahoo_shop_search[:products].count > 0
			Markets::ProductCardsFetch.new(buybid_market_name).on_product(yahoo_shop_search)
		rescue
