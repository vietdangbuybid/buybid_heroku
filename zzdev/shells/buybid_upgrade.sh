...
env="development"
echo ""
echo "  Buybid.jp Ruby upgrade, invoked from: [${green}${PWD}${reset}]"
echo ""
CURRENT_DIRECTORY="${PWD}"
cd "${PWD}/$(dirname "$BASH_SOURCE")/../../"
SOURCE_ROOT_DIR="${PWD}"

echo "${red}Running buybid_upgrade will migrate your to new version.${reset}"
echo ""
...
copy_credentails()
{
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/credentials.yml.enc" "$SOURCE_ROOT_DIR/buybid_admin/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/master.key" "$SOURCE_ROOT_DIR/buybid_admin/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/credentials.yml.enc" "$SOURCE_ROOT_DIR/buybid_batch/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/master.key" "$SOURCE_ROOT_DIR/buybid_batch/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/credentials.yml.enc" "$SOURCE_ROOT_DIR/buybid_frontend/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/master.key" "$SOURCE_ROOT_DIR/buybid_frontend/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/credentials.yml.enc" "$SOURCE_ROOT_DIR/buybid_sample/config/"
	cp -f "$SOURCE_ROOT_DIR/buybid_setup/config/master.key" "$SOURCE_ROOT_DIR/buybid_sample/config/"
}
...
echo "  - Begin upgrade" 
echo "  - Begin upgrade" >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
...
then
	...
	echo "     * bundle exec rails db:migrate RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1 "
	spring stop >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
	bundle exec rails db:migrate RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1 >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
	...
	echo "     * bundle exec rails RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1"
	spring stop >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
	bundle exec rails RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1 
else
	exit_error
fi
...
do
  ...
  echo "" >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
	...
	then
		echo "     * bundle exec rails db:migrate RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1"
		spring stop >> "$SOURCE_ROOT_DIR/zzdev_shells.log"
		bundle exec rails db:migrate RAILS_ENV=$env DISABLE_DATABASE_ENVIRONMENT_CHECK=1 
